...
def main():
    ...
    try:
        ...
        xvals = np.linspace(-.05, .05,100)
        yvals = np.linspace(-.05, .05,100)
        ...
        # Return to center
        center_frame = moveXY(0, 0)
        if controller.send_frame_with_verification(center_frame):
            frame_count += 1
        else:
            failed_frames += 1
            print(f"Failed to send frame {frame_count + failed_frames}")
        
        print(f"Sent {frame_count} frames successfully, {failed_frames} failed")
        ...
        try:
            # Send zero frame
            center_frame = moveXY(0, 0)
            controller.send_frame_with_verification(center_frame)
            print("Sending zero frames")
            while True:
                # your work here
                controller.send_frame_with_verification(center_frame)
                time.sleep(0.1)
        except KeyboardInterrupt:
            print("\nFinal interrupt - shutting down")
        controller.close()
